...
##############################################################################
# mySup = sup.Sup()
# mySup.print_list_hex_byte(mySup.send(0, [0x7D,0xBF]))
# mySup.print_list_hex_byte(mySup.send(1, []))
# mySup.print_list_hex_byte(mySup.send(2, [0x12,0x34,0x56]))

# uart_port.write(mySup.send(0, [0x7D,0xBF]))
# uart_port.write(mySup.send(1, []))
# uart_port.write(mySup.send(2, [0x12,0x34,0x56]))
...
try:
	while True:
		...
		if (temp - oldtime) >= 0.01:
			...
			# mySup.print_list_hex_byte(mySup.send(DATA_TYPE, [data]))
			uart_port.write(mySup.send(DATA_TYPE, [data]))
		...
		if ringbuff.count_item(0x7E):
			...
			if type(temp) == tuple:
				...
				if (temp[0] == DATA_TYPE) and (len(temp[1]) == 1) and (temp[1][0] == data):
					wait_for_return = False
